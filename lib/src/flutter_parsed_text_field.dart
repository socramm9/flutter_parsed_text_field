...
class FlutterParsedTextField extends StatefulWidget {
  ...
  final List<String> Function() getMentions;

  final Function(String) onMentionedRemoved;
  ...
  const FlutterParsedTextField(
      ...
      required this.getMentions,
      required this.onMentionedRemoved})
      : super(key: key);
        ...
        child: RawKeyboardListener(
          ...
          onKey: (event) {
            if (event.logicalKey == LogicalKeyboardKey.backspace &&
              ...
              //var suggestions = ["Ironman"];
              var start = _controller.selection.start;
              for (final sug in widget.getMentions()) {
                if (start - sug.length - 1 >= 0) {
                  var sub =
                      _controller.text.substring(start - sug.length - 1, start);
                  if (sub == "@" + sug) {
                    widget.onMentionedRemoved(sug);
                    _controller.text = _controller.text
                        .replaceRange(start - sug.length - 1, start, "");
                    _controller.selection = TextSelection.fromPosition(
                        TextPosition(offset: start - sug.length - 1));
                  }
                }
              }
            }
